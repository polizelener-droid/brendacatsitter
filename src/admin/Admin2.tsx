  ...
  const saveCat = async () => {
    ...
    try {
      const response = await fetch('/api/cats', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ password, cat: selected }),
      });
      const result = await response.json();
      ...
      <div className="max-w-6xl mx-auto">
        <header className="flex flex-wrap items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-full bg-[#275240] text-white flex items-center justify-center"><Cat /></div>
            <div><h1 className="text-2xl font-bold">Nuevo panel de gatos</h1><p className="text-sm">Agregá los gatos uno por uno.</p></div>
          </div>
        ...
        <section className="grid lg:grid-cols-[1.1fr_.9fr] gap-6">
          <div className="bg-white rounded-2xl border border-[#d0dacb] p-5 shadow-sm">
            <div className="flex items-center justify-between mb-4"><h2 className="font-bold text-lg">Catálogo</h2><button onClick={addNew} className="px-4 py-2 rounded-xl bg-[#275240] text-white text-sm font-bold">+ Nuevo gato</button></div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {allVisible.map((cat) => (
                <button key={cat.id} onClick={() => cats.some((c) => c.id === cat.id) && editCat(cat as StoredCat)} className="text-left rounded-xl border border-[#d0dacb] overflow-hidden bg-[#f7f9f5]">
                  <div className="aspect-square bg-[#e2e8dc]">{cat.image && <img src={cat.image} alt={cat.name} className="w-full h-full object-cover" />}</div>
                  <div className="p-3"><p className="font-bold">{cat.name}</p><p className="text-xs opacity-70">{cats.some((c) => c.id === cat.id) ? 'Nuevo catálogo' : 'Catálogo actual'}</p></div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-[#d0dacb] p-5 shadow-sm h-fit">
            ...
            <input value={selected.name} onChange={(e) => setSelected({ ...selected, name: e.target.value })} className="w-full mb-3 px-3 py-2 rounded-xl bg-[#e2e8dc] border-0" placeholder="Ej. Mishi" />
            <label className="block text-xs font-bold mb-1">Familia</label>
            <input value={selected.owner} onChange={(e) => setSelected({ ...selected, owner: e.target.value })} className="w-full mb-3 px-3 py-2 rounded-xl bg-[#e2e8dc] border-0" placeholder="Familia de Mishi" />
            <label className="block text-xs font-bold mb-1">Foto</label>
            <label className="flex items-center justify-center gap-2 w-full h-28 rounded-xl border-2 border-dashed border-[#b9c8b7] cursor-pointer bg-[#f7f9f5] mb-3 overflow-hidden">
              {selected.image ? <img src={selected.image} alt="Vista previa" className="w-full h-full object-cover" /> : <><ImagePlus size={20} /><span className="text-sm">Elegir foto</span></>}
              <input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && handleImage(e.target.files[0])} />
            </label>
            <label className="block text-xs font-bold mb-1">Historia</label>
            <textarea value={selected.story} onChange={(e) => setSelected({ ...selected, story: e.target.value })} className="w-full mb-3 px-3 py-2 rounded-xl bg-[#e2e8dc] border-0 min-h-24" />
            <label className="block text-xs font-bold mb-1">Actividad favorita</label>
            ...
            <div className="flex gap-2">
              ...
              {selected.name && cats.some((c) => c.id === selected.id) && <button disabled={saving} onClick={() => deleteCat(selected.id)} className="px-4 rounded-xl border border-red-200 text-red-700"><Trash2 size={17} /></button>}
            </div>
          </div>
        </section>
      </div>
