...
const mergeAdminCats = (remoteCats: StoredCat[]): StoredCat[] => {
  const valid = remoteCats.filter((cat) => cat?.id && cat?.name);
  if (valid.filter((cat) => cat.image).length >= 20) return valid;

  const overrides = new Map(valid.map((cat) => [cat.id, cat]));
...
export const Admin2: React.FC = () => {
  ...
  const unlock = async () => {
    ...
    try {
      ...
      let d: { error?: string } = {};
      try { d = text ? JSON.parse(text) : {}; } catch { throw new Error('El servidor local no está listo. Reiniciá npm run dev e intentá de nuevo.'); }
