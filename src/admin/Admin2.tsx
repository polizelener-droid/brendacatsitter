...
export const Admin2: React.FC = () => {
  ...
  const loadCats = async () => {
    try {
      const r = await fetch('/api/cats', { cache: 'no-store' }); const data = await r.json();
      if (!r.ok) throw new Error(data?.error || 'No se pudo cargar el catálogo.');
      setCats(Array.isArray(data.cats) ? data.cats : []);
    } catch (e) { setStatus(e instanceof Error ? e.message : 'No se pudo cargar el catálogo.'); }
  };
  const unlock = async () => {
    if (!password) return setStatus('Ingresá la clave.'); setStatus('Verificando…');
    try { const r = await fetch('/api/cats', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({action:'auth',password}) }); const d=await r.json(); if(!r.ok) throw new Error(d?.error||'Clave incorrecta.'); setUnlocked(true); setStatus(''); await loadCats(); }
    catch(e){ setStatus(e instanceof Error ? e.message : 'Clave incorrecta.'); }
  };
  const saveCat = async () => {
    if (!selected.name.trim()) return setStatus('Poné el nombre del gato.'); if (!password) return setStatus('Ingresá la clave del panel.');
    setSaving(true); setStatus('Guardando…');
    try { const r=await fetch('/api/cats',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password,cat:selected})}); const d=await r.json(); if(!r.ok) throw new Error(d?.error||'No se pudo guardar.'); setCats(d.cats||[]); setSelected(emptyCat()); setStatus('Gato guardado.'); }
    catch(e){setStatus(e instanceof Error?e.message:'No se pudo guardar.');} finally{setSaving(false);}
  };
  const deleteCat = async (id:string) => {
    if(!confirm('¿Eliminar este gato?')) return; setSaving(true); setStatus('Eliminando…');
    try { const r=await fetch('/api/cats',{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({password,id})}); const d=await r.json(); if(!r.ok) throw new Error(d?.error||'No se pudo eliminar.'); setCats(d.cats||[]); setSelected(emptyCat()); setStatus('Gato eliminado.'); }
    catch(e){setStatus(e instanceof Error?e.message:'No se pudo eliminar.');} finally{setSaving(false);}
  };
  const handleImage=(file:File)=>{const reader=new FileReader();reader.onload=()=>setSelected(p=>({...p,image:String(reader.result||'')}));reader.readAsDataURL(file);};
