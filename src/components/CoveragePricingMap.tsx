...
type NeighborhoodTile = { name: string; zone: ZoneKey; };

const zoneStyles: Record<ZoneKey, { tile: string; dot: string; card: string; title: string; button: string }> = {
  zone1: {
    tile: 'border-[#a8c9df] bg-[#dff1fb] text-[#3e6f8f]', dot: 'bg-[#8fc1df]', card: 'border-[#a8c9df] bg-[#f4fbff]', title: 'text-[#4a7ea0]', button: 'bg-[#4a7ea0] hover:bg-[#3e6f8f]',
  },
  zone2: {
    tile: 'border-[#dfb4c9] bg-[#f8ddea] text-[#935b77]', dot: 'bg-[#e4a9c6]', card: 'border-[#dfb4c9] bg-[#fff6fa]', title: 'text-[#a46a87]', button: 'bg-[#935b77] hover:bg-[#7e4c65]',
  },
};
...
const zones = [
  { key: 'zone1' as const, name: 'Zona 1', prices: [['Lunes a viernes', '$18.000'], ['Sábados', '$21.000'], ['Domingos y feriados', '$25.000']], contactName: 'Bren', whatsappUrl: 'https://wa.me/5491161386748?text=Hola%20Bren%2C%20quer%C3%ADa%20consultar%20disponibilidad%20para%20la%20Zona%201.' },
  { key: 'zone2' as const, name: 'Zona 2', prices: [['Lunes a viernes', '$20.000'], ['Sábados', '$23.000'], ['Domingos y feriados', '$27.000']], contactName: 'Bren', whatsappUrl: 'https://wa.me/5491161386748?text=Hola%20Bren%2C%20quer%C3%ADa%20consultar%20disponibilidad%20para%20la%20Zona%202.' },
];
      ...
      <div className="mt-5 grid items-start gap-5 lg:grid-cols-[1.3fr_1fr]">
        <div>
          <div className="relative overflow-hidden rounded-3xl border border-[#275240]/10 bg-[#f7f7f3] p-4">
            ...
            <div className="relative flex flex-wrap gap-2.5">{coverageTiles.map((tile) => <Tile key={tile.name} tile={tile} />)}</div>
          </div>
          <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-sm font-extrabold">
            <span className={`inline-flex items-center gap-2 ${zoneStyles.zone1.title}`}><span className={`h-3 w-3 rounded-full ${zoneStyles.zone1.dot}`} /> Zona 1</span>
            <span className={`inline-flex items-center gap-2 ${zoneStyles.zone2.title}`}><span className={`h-3 w-3 rounded-full ${zoneStyles.zone2.dot}`} /> Zona 2</span>
          </div>
        </div>
        <div className="space-y-3">
          {zones.map((zone) => <div key={zone.key} className={`rounded-3xl border p-4 ${zoneStyles[zone.key].card}`}>
            <div className="mb-3 flex items-center gap-2.5"><PawPrint className={`h-5 w-5 ${zoneStyles[zone.key].title}`} aria-hidden="true" /><h4 className={`font-display text-xl font-black sm:text-2xl ${zoneStyles[zone.key].title}`}>{zone.name}</h4></div>
            <div className="space-y-2">{zone.prices.map(([label, price]) => <div key={label} className="flex items-center justify-between gap-3 text-xs sm:text-sm"><span className="font-semibold text-[#275240]/75">{label}</span><strong className="font-black text-[#275240]">{price}</strong></div>)}</div>
          </div>)}
          <a href="https://wa.me/5491161386748?text=Hola%20Bren%2C%20quer%C3%ADa%20consultar%20disponibilidad%20para%20una%20visita." target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 rounded-xl bg-[#4a7ea0] px-4 py-2.5 text-sm font-bold text-white transition hover:bg-[#3e6f8f]"><WhatsAppIcon className="h-5 w-5 fill-white text-white" />Hablar con Bren</a>
        </div>
      </div>
