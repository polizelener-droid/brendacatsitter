      ...
      <div className="mt-5 grid items-start gap-5 lg:grid-cols-[1.3fr_1fr]">
        ...
        <div className="space-y-3">
          {zones.map((zone) => <div key={zone.key} className={`rounded-3xl border p-4 ${zoneStyles[zone.key].card}`}>
            ...
            <a href={zone.whatsappUrl} target="_blank" rel="noopener noreferrer" className={`mt-4 flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold text-white transition ${zoneStyles[zone.key].button}`}><WhatsAppIcon className="h-5 w-5 fill-white text-white" />Hablar con {zone.contactName}</a>
          </div>)}
        </div>
      </div>
