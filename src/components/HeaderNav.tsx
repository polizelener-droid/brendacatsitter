...
export const HeaderNav: React.FC = () => {
  ...
  const navLinks = [
    ...
    { name: 'Michis cuidados', href: '#fotos' },
    ...
    <header className={`fixed left-0 right-0 top-0 z-50 transition-all duration-300 ${isScrolled ? 'border-b border-[#275240]/10 bg-[#e2e8dc]/95 py-2 shadow-xs backdrop-blur-md' : 'bg-transparent py-2.5'}`}>
      <div className="mx-auto max-w-7xl px-5 sm:px-7 lg:px-9">
        <div className="flex items-center justify-end gap-3">
          ...
          <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} id="mobile-menu-toggle" className="shrink-0 rounded-xl border border-[#275240]/10 bg-white p-2 text-[#275240] shadow-2xs transition-colors hover:bg-[#e2e8dc] lg:hidden" aria-label="Menú">
            {mobileMenuOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
          </button>
        </div>
        {mobileMenuOpen && <div className="mt-3 rounded-2xl border border-[#275240]/10 bg-[#e2e8dc]/98 px-3 pb-4 pt-3 shadow-xl backdrop-blur-xl lg:hidden"><div className="flex flex-col gap-1">{navLinks.map((link) => <a key={link.name} href={link.href} onClick={(e) => handleNavClick(e, link.href)} className="rounded-xl px-3.5 py-2 text-sm font-semibold text-[#275240] transition-colors hover:bg-white/60">{link.name}</a>)}</div></div>}
      </div>
    </header>
  );
};
